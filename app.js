...
let mainData = [];
let meineKarte = L.map("karte").setView([51.16229, 6.462739], 2);
...
function set_wind_definition(windspeed) {
  ...
  if (windspeed >= 62 && windspeed <= 74) {
    ...
    document.getElementById("wind_container").style.background =
      "linear-gradient(to bottom, transparent,yellow)";
  }
  ...
  fetch(apiLink)
    ...
    .then((data) => {
      ...
      if (data.alerts) {
        ...
        btnShowMoreAlerts.addEventListener("click", () => {
          ...
          outpAlertHeadline.innerHTML =
            "Wetteralarm " + (showAlert + 1) + "/" + alertAmount;
          document.getElementById("outpAlertTitle").innerHTML = alertEvent;
          document.getElementById("outpAlertMessage").innerHTML = alertBody;
          document.getElementById("outpAlertStart").innerHTML =
            "Start: " + alertStart;
          document.getElementById("outpAlertEnd").innerHTML =
            "Ende: " + alertEnd;
          document.getElementById("outpAlertSource").innerHTML =
            "Quelle: " + alertSender;
        });
      ...
      outSun.innerHTML =
        ...
        sunrise +
        ` | <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="orange" class="bi bi-sunset-fill" viewBox="0 0 16 16">
      ...
      if (
        ...
        }px; top: -8px; background-Color: yellow;}`;
      }
      ...
      for (let i = 0; i <= 6; i++) {
        ...
        if (i === 0) {
          const tempDiffToday_Tomorrow = (
            data.daily[i + 1].temp.max - data.daily[i].temp.max
          ).toFixed(1);
          const lblTempDiff = document.getElementById("outpTempDiff");
          const maxTomorrow = data.daily[i + 1].temp.max;
          //* accumulate rain
          if (data.daily[i + 1].rain !== undefined) {
            rain_sum += data.daily[i + 1].rain;
          }
          //* accumulate temp day
          average_temp_day += data.daily[i + 1].temp.max;
          //* accumulate temp night
          average_temp_night += parseFloat(data.daily[i + 1].temp.night);
...
function ausw(param_current_Temp) {
  ...
  if (param_current_Temp >= 32) {
    ...
    document.getElementById("outpTemp").style.textShadow =
      "0px 0px 15px orange";
...
function loadData() {
  ...
  if (localStorage.getItem("stored_WeatherSettings") != null) {
    ...
    try {
      ...
      document.getElementById("settingsAppearance").value =
        weatherSettings.appeareanceMode;
    } catch (error) {
      setAppearance("opt_normalmode");
    }
  } else {
  ...
  fetch(pollutionLink)
    ...
    .then((data) => {
      ...
      const airQualityColor = [
        "transparent",
        "green",
        "rgba(144, 238, 144, 0.678)",
        "rgba(255, 196, 0, 0.669)",
        "rgba(180, 89, 9, 0.938)",
        "red",
      ];
